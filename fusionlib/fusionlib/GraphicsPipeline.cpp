...
void GraphicsPipeline::Setup() {
	...
	cout << "bindings:" << bindings.size() << endl;
	...
	binder = new UniformBinder(swapChainFramebuffers.size(), uni, desLay, Diff);
	binder->Update();

}

GraphicsPipeline::GraphicsPipeline(FusionApp * app,Effect * fx,VertexBuffer * vb,Texture2D * diff)
{
	Diff = diff;
